...
def get_loss_fn(noise, graph, train, sampling_eps=1e-3, lv=False):
    ...
    def loss_fn(model, batch, cond=None, t=None, perturbed_batch=None):
        ...
        loss = dsigma[:, None] * loss # Modified to exclude EOS tokens when calculating PPL

        return loss

    return loss_fn

def get_loss_fn_conditional(noise, graph, train, sampling_eps=1e-3, lv=False):

    def loss_fn(model, batch, infill_mask, cond=None, t=None, perturbed_batch=None):
        """
        Batch shape: [B, L] int. D given from graph
        """
        ...
        # Use the unmasked tokens
        perturbed_batch = perturbed_batch * infill_mask + batch * (1 - infill_mask)
